...
train_lite <- select(train, -RATE_PLAN, -HANDSET_NAME, -VALUE_SEGMENT, -GENDER)
test_lite <- select(test, -RATE_PLAN, -HANDSET_NAME, -VALUE_SEGMENT, -GENDER)
```
...
```{r}
set.seed(1337)
model.pls2 <- plsda(select(train_lite, -CONTRACT_KEY, -TARGET),
                    train_lite$TARGET,
                    probMehod = "Bayes",
                    na.action = na.pass,
                    ncomp = 13)
...
```{r}
set.seed(1337)
alternate_train <- downSample(select(train_lite, -CONTRACT_KEY, -TARGET), train$TARGET, yname = "TARGET")
model.pls <- train_lite(TARGET ~ .,
                   alternate_train,
                   method = "pls",
...
```{r}
set.seed(1337)
alternate_train <- upSample(select(train_lite, -CONTRACT_KEY, -TARGET), train_lite$TARGET, yname = "TARGET")
model.pls <- train_lite(TARGET ~ .,
                   alternate_train,
                   method = "pls",
                   tuneGrid = expand.grid(.ncomp = 1:20),
...
```{r}
pred.pls <- predict(model.pls, newdata = test_lite)
summary(pred.pls)
```
...
```{r}
set.seed(1337)
alternate_train <- downSample(select(train_lite, -CONTRACT_KEY, -TARGET), train_lite$TARGET, yname = "TARGET")
