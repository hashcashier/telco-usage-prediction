...
```{r}
handsetTargetData <- train %>%
  group_by(HANDSET_NAME) %>%
  summarise(HANDSET_TARGET_RATIO = sum(TARGET)/length(CONTRACT_KEY))
handsetTargetRatio <- function(dataset) {
  merge(dataset, handsetTargetData, all.x = TRUE) %>%
    select(-HANDSET_NAME)
}
```
