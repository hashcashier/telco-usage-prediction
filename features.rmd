...
```{r}
subs_train  <- train %>% select(GENDER, VALUE_SEGMENT)
subs_test   <- test %>% select(GENDER, VALUE_SEGMENT)
subs_data   <- rbind(subs_train, subs_test)

mca <- MCA(subs_data, graph = FALSE)
mca_vars <- data.frame(mca$var$coord)
mca_obs <- data.frame(mca$ind$coord)
...
ggplot(data = mca_obs, aes(x = Dim.1, y = Dim.2)) +
  ...
  geom_text(data = mca_vars, aes(x = Dim.1, y = Dim.2, label = rownames(mca_vars))) +
