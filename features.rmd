...
combine <- function(dataset) {
  ...
  dataset$ROAMING_SESSIONS_206[is.na(dataset$ROAMING_USAGE_206)] <- 0
...
#   select(#starts_with("VALUE_SEGMENT"),
#          #starts_with("GENDER"),
#          starts_with("HANDSET_NAME"))
# }
# 
# cumulVar <- function (pr) {
#   cumsum(pr$sdev^2 / sum(pr$sdev^2))
# }
# 
# pca <- prcomp(colSelect(train), center = FALSE, scale = FALSE)

reduceDim <- function (dataset) {
  #res <- predict(pca, newdata = colSelect(dataset))[, 1:27]
  binaryIndicators(dataset) #%>%
    #select(-starts_with("HANDSET_NAME"))# %>%
    #cbind(res)
}
